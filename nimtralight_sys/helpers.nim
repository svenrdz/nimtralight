...
proc `$`*(s: ULString | CString): string =
  for i in 0..<s.len:
    result.add $(char(s.data[i]))

proc initConfig: CConfig =
...
proc newConfig*: ULConfig =
  result = initConfig()

proc initSettings: CSettings =
